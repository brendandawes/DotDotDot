...
public class DataSwarm extends PApplet {
...
final float STROKE_WEIGHT = 1.5f;

final HColorPool colors = new HColorPool(0xffFFFFFF, 0xff71a9cc, 0xffc5413d, 0xffbcbc9a,0xffc5d7ef,0xff3aaca3,0xffcbbf71);

public void setup() {
  size(1024,768);
  RG.init(this);
  H.init(this).background(0xff282828).autoClear(false);
  smooth();
  //grp = RG.loadShape("datapoetry.svg");
  //grp = RG.centerIn(grp, g);
  grp = RG.getText("DATA", "DinMedium.ttf", 272, CENTER);
  //grp = RG.centerIn(grp, g);
...
public void createSwarm(float x, float y) {
    ...
    .setOnCreate (
        new HCallback() {
          ...
          d
            ...
            .loc(pool.count()*(width/points.length),random(height))
            .anchorAt( H.CENTER )
            .alpha(0)
          ;
...
public void addProject(String title, String url, int date) {
...
public void draw() {
 ...
 timer = millis() - timerStart;
  ...
  while(it.hasNext()) {
    ...
    float snappedYpos = floor( d.y() / GRID_SIZE) * GRID_SIZE;
    float r = d.width();
    fill(d.fill());
    noFill();
...
public static class HEllipse extends HDrawable {
	...
	public HEllipse radius(float radius) {
		size(radius*2);
		return this;
	}
...
public static class HRect extends HDrawable {
	protected float _tl, _tr, _bl, _br;
...
public static class HText extends HDrawable {
	...
	protected void adjustMetrics() {
		...
		_width = (_text==null)? 0 : app.textWidth(_text);
...
class Project {
	...
	Project (String title, String url, int date) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "DataSwarm" };
    if (passedArgs != null) {
